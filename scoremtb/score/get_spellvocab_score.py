...
def get_score_spell(task_data, dfid, type):
    """
    -----------------------------------------------------------------------------------------
    ...
    if type == config['spell_type']:
        task_filter = task_data[task_data['assessmentid'] == config['spell_asses_val']].reset_index()
        ...
        task_filter = task_filter[['recordid', 'steps', config['spell_THETA'], config['spell_SE']]]
        ...
        task_filter = task_data[task_data['assessmentid'] == config['vocab_asses_val']].reset_index()
        ...
        task_filter = task_filter[['recordid', 'steps', config['vocab_THETA'], config['vocab_SE']]]
        merge_df = task_filter.merge(dfid, on = 'recordid', how = 'left')
        ...
        task_filter = task_data[task_data['assessmentid'] == config['psm_asses_val']] 
        ...
        task_scrd = task_scrd[['recordid', 'steps', config['psm_adj'], config['psm_exact']]]
        merge_df = task_scrd.merge(dfid, on = 'recordid', how = 'left')
